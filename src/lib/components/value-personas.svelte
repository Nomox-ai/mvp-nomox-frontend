<script lang="ts">
	...
	function measureBtn(id: PersonaId) {
		...
		ind = { left: br.left - cr.left, top: br.top - cr.top, width: br.width, height: br.height };
	}
	...
	function selectPersona(id: PersonaId, fromCycle = false) {
		...
		measureBtn(id);
		animating = true;
		setTimeout(() => { active = id; animating = false; }, 160);
	}
...
<section class="border-border border-b">
	<div class="mx-auto max-w-section px-8 py-20">
		...
		<div data-fade-up="pending" use:fadeUp class="flex flex-wrap items-center justify-center gap-x-3 gap-y-2">
			...
			>
				...
				<span
					class="absolute rounded-full bg-primary {mounted ? 'transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]' : ''}"
			...
			<span class="text-foreground text-3xl tracking-tight">love Nomox.</span>
		</div>
		...
		<div class="mt-8 flex justify-center">
			<a href={current.href} class="text-muted-foreground hover:text-foreground text-sm transition-colors">
