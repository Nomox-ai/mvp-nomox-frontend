<script lang="ts">
	...
	const productItems: { title: string; href: string; description: string }[] = [
		...
		},
		{
			...
			href: '/product#mcp',
			description: 'Expose your catalog to Claude, Cursor and any MCP-compatible AI client with one toggle.'
		},
		{
			title: 'BI platform',
			href: '/product#bi',
			description: 'Explore and visualise your data with a built-in analytics layer powered by your semantic model.'
		}
	];
	...
	const solutionItems: { title: string; href: string; description: string }[] = [
		{
			...
			description: 'Give analysts a shared vocabulary without manual tagging or lengthy governance projects.'
		},
		{
			title: 'Decision Makers',
			href: '/solutions#decision-makers',
			description: 'Let executives explore and understand their data landscape without needing to know SQL or the schema.'
		},
		{
...
<header class="border-border bg-background relative z-10 border-b">
	...
	{#if menuOpen}
		<div transition:slide={{ duration: 220 }} class="border-border bg-background absolute inset-x-0 top-full border-t shadow-lg md:hidden">
			<div class="mx-auto max-w-5xl px-8 py-5 flex flex-col gap-1">

				<p class="text-muted-foreground mb-2 text-[10px] font-semibold uppercase tracking-widest">
					Solution
				</p>
				{#each solutionItems as item}
					<a
				...
				<p class="text-muted-foreground mb-2 mt-5 text-[10px] font-semibold uppercase tracking-widest">
					Products
				</p>
				{#each productItems as item}
					<a
